terraform {
  backend "s3" {
    bucket = "should be written bucket name"
    key = "should be written path that will ve created in s3 bucket such as         env/dev/tf-remote-backend.tfstate"
    region = "us-east-1"
    dynamodb_table = "should be written dynamodb_table name"
    encrypt = true
  }
}
...
resource "aws_s3_bucket_versioning" "name such as;   versioning_backend_s3" {
...
resource "aws_dynamodb_table" "tf-remote-state-lock" {
  hash_key = "LockID"
  name     = "should be written dynamodb_table name"
